...
- App 底部全局 `Settings` 入口提供"Connection mode"三态开关：`Auto`（推荐，优先直连，必要时使用 Relay）/ `Direct only`（直连建立后 session payload data 不由 Relay 承载，直连不可用时失败）/ `Relay only`（固定使用 relay path，session payload data 仍保持加密）。选中即写入 `AsyncStorage["omniwork.transportPreference"]` 并触发当前 pairing 重连，使偏好通过 `mobile.connect.transport_preference` 即时生效；设备主界面会用 `Direct` / `Relay assisted` 徽标提示当前实际连接路径。
