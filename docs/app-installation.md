...
OMNIWORK_DEFAULT_RELAY_URL=wss://relay.company.example/relay/ws/mobile
```
...
- App 不使用 SSO，配对页输入 Mac Agent 启动生成的 32 字符临时 key。
- iOS/Android 真机可以访问 Relay。
- 移动端交付物是 APK/IPA 安装包；Web 端交付物是静态 SPA，不作为 PWA 或扫码入口。

### Relay URL 环境变量约定

Mac Agent 与 App 各自读取独立的 Relay URL，分别指向 Relay 的 `/relay/ws/agent` 与 `/relay/ws/mobile` 两个 pool；二者不可混用：

| 变量名 | 使用方 | Relay 路径 | 必填/示例 |
| --- | --- | --- | --- |
| `OMNIWORK_RELAY_URL` | Mac Agent 自连 | `/relay/ws/agent` | 必填，缺失时 Agent 启动失败；示例：`wss://relay.company.example/relay/ws/agent` |
| `OMNIWORK_DEFAULT_RELAY_URL` | App（native + web）默认值 | `/relay/ws/mobile` | `wss://relay.company.example/relay/ws/mobile` |
...
| 变量名 | 使用方 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `OMNIWORK_AGENT_CONNECTION_HEARTBEAT_MS` | Mac Agent | `10000` | 超过该时间未收到消息后连接可能进入 idle |
| `OMNIWORK_AGENT_CONNECTION_STALE_MS` | Mac Agent | `30000` | 超过该时间未收到消息后连接进入 stale |
| `OMNIWORK_AGENT_CONNECTION_DISCONNECT_MS` | Mac Agent | `90000` | 超过该时间未收到消息后连接进入 disconnected |

> 配对二维码中的 `relay_url` 由 Agent 端基于 `OMNIWORK_RELAY_URL` 推导（自动改写为兄弟路径 `/relay/ws/mobile`），手机扫码后会覆盖 App 的默认值。
