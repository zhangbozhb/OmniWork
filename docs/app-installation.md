...
- App `Devices` 页提供"Transport preference"三态开关：`Auto`（默认，跟随 Relay 灰度）/ `Prefer P2P`（跳过灰度，要求升级到 WebRTC）/ `Relay Only`（禁用 P2P 升级，永远走 relay path）。选中即写入 `AsyncStorage["omniwork.transportPreference"]` 并触发当前 pairing 重连，使偏好通过 `mobile.connect.transport_preference` 即时生效。
