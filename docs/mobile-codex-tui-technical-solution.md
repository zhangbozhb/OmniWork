...
- 手机端采用跨端移动 App 技术，同时适配 Android 和 iOS。
- 手机端推荐采用 React Native + Expo / Expo Dev Client + TypeScript。
...
- App 最终交付 Android APK 和 iOS IPA 安装包；不是网页、PWA 或浏览器入口。

这意味着此前「手机 PWA 优先」和「Mac 企业版迁移 Rust/Swift」不再作为主路线。手机端只按 APK/IPA App 交付，Rust/Swift 只作为 Relay 可选实现或极薄 macOS 原生桥接。
...
`@xterm/xterm` 当前稳定包是 6.0.0，是成熟的浏览器端终端组件。不过在本项目当前 App 目标下，MVP 不把浏览器终端作为默认实现，避免把交付形态拉回网页/PWA 思路。
...
- 如果后续需要完整 ANSI 行为，再以可替换的原生 terminal renderer 引入，不能回退成网页交付入口。
...
手机端第一版建议使用 React Native + Expo / Expo Dev Client 作为跨端移动技术，同时交付 Android 和 iOS。

技术判断：

- Android 和 iOS 共享同一套 TypeScript 业务代码。
...
- 交付物必须是 APK / IPA 安装包；不提供 PWA 作为主入口。
...
| 领域 | 推荐 | 备选 | 不推荐作为主线 |
| --- | --- | --- | --- |
| 手机端 | React Native + Expo / Expo Dev Client + TypeScript，产出 APK/IPA | Flutter | 只做 PWA 或网页作为主交付 |
...
```text
Phone:
  React Native + Expo Dev Client / bare workflow
  TypeScript
