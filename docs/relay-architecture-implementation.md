...
- App UI：底部全局 `Settings` 入口中，`transportPreference` 以 “Connection mode” 展示为 `Auto` / `Direct only` / `Relay only`，弱化协议名并强调当前连接路径的隐私含义（直连建立后 session payload data 不由 Relay 承载）；设备主界面订阅 `transport.onPathChange()`，在 active device 上用 `Direct` / `Relay assisted` 徽标提示当前实际路径。用户切换到 `Direct only`（底层仍为 `prefer_p2p`）时弹 `Switch to Direct only?` 二次确认，确认后才写入 `AsyncStorage["omniwork.transportPreference"]` 并触发当前 pairing 重连。`onForceClose` 通过 `formatStrictForceCloseMessage(reason)` 把 `strict_unavailable:relay_disabled` / `:blocklisted` / `:backoff_active` 以及 `peer_unavailable` / `create_offer_failed` / `handle_offer_failed` / `handle_answer_failed` / `timeout` / `pong_timeout` / `ice_failed` / `ice_disconnected` / `buffered_overflow` / `peer_closed` / `peer_missing` 等 reason 翻译成面向用户的 Direct connection 状态文案。
