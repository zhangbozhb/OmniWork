...
- Agent 通过 `agent.hello` 注册 `device_id`、协议版本和能力；Relay 鉴权通过后分配 `agent_connection_id`，并保证同一 `device_id` 只有一个在线 Agent。其中 workspace 只使用缓存快照，进入工作台或用户显式刷新时由 `session.list` 刷新 session 视图和 workspace 摘要，`workspace.list` 仅保留为独立 workspace 列表刷新入口。
