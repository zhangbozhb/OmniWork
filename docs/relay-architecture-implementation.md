...
- [e2e-noise-roadmap.md](./e2e-noise-roadmap.md)：App-Agent E2E 加密的设计基线与落地阶段。
...
本文不再维护按日期或工期划分的总体路线图，也不保留废弃公网隧道、历史 spike、旧 P2P 迁移计划等过程信息。

本文只记录当前 Relay 架构的实施状态和后续演进边界，用于回答：

- 当前 Relay 代码应当满足哪些架构约束。
- 哪些能力已经落地，哪些能力仍在推进。
- P2P 与 E2E 加密在实施顺序上的关系。
- 后续修改 Relay、App、Agent、protocol 时需要保持哪些不变量。
...
当前项目不是从一开始就同时完成 P2P 与 E2E 加密，而是按以下顺序演进：
...
因此，本文中的 E2E 约束描述的是当前安全改造后的目标态；P2P 相关模块和文档仍保留，因为它们是已经先行落地的传输基础。
...
- 当前文档统一表述为：部署方提供稳定公网入口，业务安全由 App-Agent E2E 负责。
...
P2P 已经作为传输能力落地；E2E 改造阶段对 P2P 控制面做了如下收口：

- relay path 和 p2p path 使用统一 E2E 安全模型。
- `tunnel.upgrade.propose` 仅作为 Relay 定向升级提示；后续信令走 E2E。
