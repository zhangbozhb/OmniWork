...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([loadPairings(), Linking.getInitialURL()])
      .then(async ([savedPairings, initialUrl]) => {
        ...
        if (scannedPairing) {
          ...
          setConnectionMessage("Pairing imported from QR code. Connecting...");
          return;
        }
  ...
  useEffect(() => {
    const subscription = Linking.addEventListener("url", ({ url }) => {
      handlePairingUrl(url).catch((error: unknown) => {
        setPairingError(
          `Could not import pairing QR: ${formatErrorMessage(error)}`,
        );
      });
    });
  ...
  async function handlePairingUrl(url: string): Promise<void> {
    ...
    if (!nextPairing) {
      Alert.alert(
        "Invalid QR code",
        "Scan the QR code generated by the Mac Agent.",
      );
      return;
    }
