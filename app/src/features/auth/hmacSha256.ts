...
export function createHmacSha256Base64Url(key: string, message: string): string {
  const keyBytes = utf8Bytes(key);
  const normalizedKey = keyBytes.length > SHA256_BLOCK_SIZE ? sha256(keyBytes) : keyBytes;
...
function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += SHA256_BLOCK_SIZE) {
    ...
    for (let index = 0; index < 64; index += 1) {
      const t1 = add32(h, bigSigma1(e), choose(e, f, g), K[index], words[index]);
