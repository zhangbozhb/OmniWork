...
- Runs a local Agent Probe hook receiver for Codex / Claude Code / Trae / Trae CN events. Codex and Claude Code hooks use `@omniwork/surface-hook-post`; Trae and Trae CN install both `@omniwork/surface-hook-record` for local records and `@omniwork/surface-hook-post` for realtime POST delivery.
